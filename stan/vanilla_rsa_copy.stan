...
data {
  ...
  int<lower=1> utt;  // observed utterance index
  int<lower=1> tar;     // target state index
}
...
model {
  // Priors
  alpha ~ normal(5, 5);
  cost_weight ~ normal(1, 1);
  
  // Likelihood: observed utterance given target state and S1 distribution
  //utt ~ categorical(to_vector(S1[tar]));
  utt ~ normal(alpha + cost_weight * S);
